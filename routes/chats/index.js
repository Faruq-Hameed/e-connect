...
const {getByAny, getElementById,findIndexOf,} = require('../../functions') //functions to get any element with the supplied arguments
...
router.get('/:userId/:friendId', (req, res) => {
    ...
    const friend = getElementById(users, req.params.friendId) //using the provided friendId to get the friend(user) object from the database
    ...
    const userChats = getElementById(allChats, req.params.userId)//getting all the user's chats from the database(chats.js)
    
    const userChatsWithFriend = getByAny(userChats.chats,'friendId', req.params.friendId)//getting chats between the user and the friend
